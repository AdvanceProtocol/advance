...
*Setup instructions for a Gitian build of Advance Protocol using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Advance
...
- [Building Advance Protocol](#building-advance-protocol)
...
Clone the git repositories for Advance Protocol and Gitian.
...
in the Advance Protocol repository under 'Fetch and build inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Advance Protocol
----------------

To build Advance Protocol (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Advance Protocol repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/advance/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/advanceprotocol/advance
...
For example:
```bash
URL=https://github.com/crowning-/advance.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit advance=${COMMIT} --url advance=${URL} ../advance/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit advance=${COMMIT} --url advance=${URL} ../advance/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit advance=${COMMIT} --url advance=${URL} ../advance/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the advance git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../advance/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/advance.git
SIGPATH=/some/root/path/advance-detached-sigs.git

./bin/gbuild --url advance=${BTCPATH},signature=${SIGPATH} ../advance/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/advance-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/advance-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/advance-osx-build.assert
```
...
[advance/gitian.sigs](https://github.com/advanceprotocol/gitian.sigs/) repository, or if that's not possible to create a pull
request.
