 ...
 *
 *   // network
 *   CAdvanceNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.advance.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CAdvanceNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
public:
    CAdvanceNetwork(UniValue objIn)
    {
...
// // can be under: AdvanceNetwork
// //   -- signature requirements : Key1(User)
// class CAdvanceNetworkVariable : public CGovernanceObject
// {
// private:
