...
const char * const BITCOIN_CONF_FILENAME = "advance.conf";
const char * const BITCOIN_PID_FILENAME = "advanced.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "advance" is a composite category enabling all Advance-related debug output
            if(ptrCategory->count(string("advance"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\AdvanceProtocol
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\AdvanceProtocol
    // Mac: ~/Library/Application Support/AdvanceProtocol
    // Unix: ~/.advanceprotocol
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "AdvanceProtocol";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/AdvanceProtocol";
#else
    // Unix
    return pathRet / ".advanceprotocol";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty advance.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override advance.conf
